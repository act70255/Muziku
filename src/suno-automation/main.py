...
職責：CLI 入口，串接 PromptBuilder、SunoClient、Downloader、WorkflowHook。
"""
...
# 預設設定檔與元素庫路徑
DEFAULT_CONFIG = Path(__file__).parent / "config" / "settings.toml"
...
def _create_suno_client(
    ...
    *,
    ...
    return SunoClient(
        headless=headless,
        login_method=suno_cfg.get("login_method", "google"),
...
def auth_status(
    ...
    if not DEFAULT_STATE_FILE.exists():
        ...
        console.print("[yellow]請先執行 `uv run python main.py auth login`[/yellow]")
        raise typer.Exit(1)
